...
$queryId = "SELECT u.id FROM users u JOIN users_doc_spec d ON u.id = d.id WHERE u.username = '$val'";
$queryId_run = mysqli_query($conn, $queryId);
		...
		$query = "SELECT u.*,d.* FROM users u JOIN users_doc_spec d ON u.id = d.id WHERE u.id = '$valId'";
					...
					?>
							<form action="doktor_update.php" id='edit_form2' method="POST" enctype="multipart/form-data">
								<tr>
								...
								<tr>
									...
									<td><label class="form-label" for="specializimi">Specializimi</label></td>
											<td>	
												<textarea tyle="text" rows="4" cols="0" name="specializimi" id="specializimi" required placeholder="<?php echo $row["specializimi"] ?>" class="form-control" ></textarea>
											</td>
									</tr>
									<tr>
										<tr>
									<td>	<label class="form-label" for="image">Image</label></td>
												<td><input type="file" name="image" accept='image/*' id="image" class="form-control" ></td>
										</tr>
									<td></td>
